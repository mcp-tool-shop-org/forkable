...
describe("network errors are mapped to a structured ForkableError (H3)", () => {
  it("mapGitHubError: ECONNRESET becomes a structured ForkableError (not a raw Node SystemError)", () => {
    const err = mapGitHubError(econnresetError());
    expect(err.name).toBe("ForkableError");
  ...
  it("mapGitHubError: ETIMEDOUT becomes a structured ForkableError", () => {
    const err = mapGitHubError(etimedoutError());
    expect(NETWORK_ERROR_CODES).toContain(err.code);
  });

  it("mapGitHubError: AbortError becomes a structured ForkableError", () => {
