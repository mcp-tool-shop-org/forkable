 ...
 *
 * Bounds:
 *   - Snapshots under `.forkable/snapshots/rename-<ts>-<rand>/`
...
describe("snapshot — git repo", () => {
  ...
  it("records pre-rename HEAD SHA under .forkable/snapshots/rename-*/pre-head.txt", async () => {
    if (!hasGit()) return;
    fx = newFixture();
    ...
    const preHeadFile = fx.resolve(".forkable/snapshots", snap.id, "pre-head.txt");
  ...
  it("captures uncommitted changes via git stash --include-untracked", async () => {
    ...
    expect(existsSync(fx.resolve(".forkable/snapshots", snap.id, "stash-ref.txt"))).toBe(true);
  });
...
describe("snapshot — non-git repo (§5)", () => {
  ...
  it("writes a tree/ directory of the working tree when no .git is present", async () => {
    ...
    const treeDir = fx.resolve(".forkable/snapshots", snap.id, "tree");
    expect(existsSync(treeDir)).toBe(true);
    expect(existsSync(fx.resolve(".forkable/snapshots", snap.id, "tree/a.txt"))).toBe(true);
    expect(existsSync(fx.resolve(".forkable/snapshots", snap.id, "tree/src/b.txt"))).toBe(true);
  });

  it("tree excludes node_modules/, dist/, .git/, .forkable/", async () => {
    fx = newFixture();
    fx.write("a.txt", "keep\n");
    fx.write("node_modules/x/pkg.json", "skip\n");
    fx.write("dist/x.js", "skip\n");
    const snap = await takeSnapshot(fx.root);
    const base = fx.resolve(".forkable/snapshots", snap.id, "tree");
...
describe("snapshot — GC", () => {
  ...
  it("removes snapshot directories older than 7 days", async () => {
    ...
    mkdirSync(fx.resolve(".forkable/snapshots", oldId), { recursive: true });
    const recentId = `rename-${Math.floor(Date.now() / 1000)}-new000`;
    mkdirSync(fx.resolve(".forkable/snapshots", recentId), { recursive: true });

    await gcSnapshots(fx.root);

    expect(existsSync(fx.resolve(".forkable/snapshots", oldId))).toBe(false);
    expect(existsSync(fx.resolve(".forkable/snapshots", recentId))).toBe(true);
  });

  it("retains all snapshots when none are older than the cutoff", async () => {
    fx = newFixture();
    const id = `rename-${Math.floor(Date.now() / 1000)}-abc123`;
    mkdirSync(fx.resolve(".forkable/snapshots", id), { recursive: true });
    await gcSnapshots(fx.root);
    expect(existsSync(fx.resolve(".forkable/snapshots", id))).toBe(true);
  });
