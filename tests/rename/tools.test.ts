...
describe("rename-plan tool (F7)", () => {
  ...
  it("produces .forkable/rename-plan.json at the repo root", async () => {
    fx = newFixture();
    seedHardCaseRepo(fx);
    ...
    expect(existsSync(fx.resolve(".forkable/rename-plan.json"))).toBe(true);
  });

  it("produces .forkable/rename-plan.diff (unified diff preview)", async () => {
    fx = newFixture();
    seedHardCaseRepo(fx);
    ...
    expect(existsSync(fx.resolve(".forkable/rename-plan.diff"))).toBe(true);
    const diff = readFileSync(fx.resolve(".forkable/rename-plan.diff"), "utf8");
    expect(diff).toContain("# forkable rename plan diff");
...
describe("rename-apply tool (F7)", () => {
  ...
  it("returns RENAME_PLAN_STALE when the plan file is missing", async () => {
    ...
    const r = await renameApplyTool.handler(
      { path: fx.root, plan: ".forkable/does-not-exist.json", verify: false },
      makeCtx(),
    );
    expect(r.ok).toBe(false);
  ...
  it("writes a snapshot to .forkable/snapshots/rename-*/ before mutating", async () => {
    fx = newFixture();
    seedHardCaseRepo(fx);
    // Build a plan first.
    ...
    expect(existsSync(fx.resolve(".forkable/snapshots", applyRes.data.receipt.snapshotId))).toBe(true);
  });
  ...
  it("writes .forkable/rename-receipt.json alongside the snapshot", async () => {
    fx = newFixture();
    seedHardCaseRepo(fx);
    const planRes = await renamePlanTool.handler(
      {
    ...
    expect(existsSync(fx.resolve(".forkable/rename-receipt.json"))).toBe(true);
  });
  ...
  it("works on a non-git repo — snapshot mode is `files`", async () => {
    ...
    const snapDir = fx.resolve(".forkable/snapshots", applyRes.data.receipt.snapshotId);
