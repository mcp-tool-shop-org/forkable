...
describe("registry — rename tools registered", () => {
  it("TOOLS includes forkable_rename_plan", () => {
    expect(TOOLS.find((t) => t.name === "forkable_rename_plan")).toBeDefined();
  });

  it("TOOLS includes forkable_rename_apply", () => {
    expect(TOOLS.find((t) => t.name === "forkable_rename_apply")).toBeDefined();
  });

  it("TOOLS includes forkable_rename_rollback", () => {
    expect(TOOLS.find((t) => t.name === "forkable_rename_rollback")).toBeDefined();
  });

  it("findTool('forkable_rename_plan') returns the plan tool descriptor", () => {
    const t = findTool("forkable_rename_plan");
    expect(t).toBeDefined();
    expect(t!.name).toBe("forkable_rename_plan");
    expect(typeof t!.handler).toBe("function");
  });

  it("rename tools all carry non-empty descriptions", () => {
    for (const name of ["forkable_rename_plan", "forkable_rename_apply", "forkable_rename_rollback"]) {
...
describe("dispatch — rename-plan", () => {
  ...
  it("records a successful audit entry with tool=forkable_rename_plan on success", async () => {
    fx = newFixture();
    seedHardCaseRepo(fx);
    const ctx = makeCtx();
    ...
    const entries = log.query({ tool: "forkable_rename_plan", limit: 5 });
...
describe("dispatch — rename-apply", () => {
  ...
  it("surfaces RENAME_PLAN_STALE when the plan file is missing", async () => {
    ...
    const r = await dispatch(
      renameApplyTool,
      { path: fx.root, plan: ".forkable/missing.json", verify: false },
      ctx,
    );
    expect(r.ok).toBe(false);
    ...
    const log = new AuditLog(ctx.db);
    const entries = log.query({ tool: "forkable_rename_apply", limit: 5 });
    expect(entries).toHaveLength(1);
    expect(entries[0]!.ok).toBe(false);
  });
...
describe("dispatch — rename-rollback", () => {
  ...
  it("records a failed audit entry for RENAME_ROLLBACK_NOT_FOUND", async () => {
    ...
    const log = new AuditLog(ctx.db);
    const entries = log.query({ tool: "forkable_rename_rollback", limit: 5 });
    expect(entries).toHaveLength(1);
    expect(entries[0]!.ok).toBe(false);
  });
  ...
  it("records a success audit entry after a successful rollback", async () => {
    ...
    const log = new AuditLog(ctx.db);
    const entries = log.query({ tool: "forkable_rename_rollback", limit: 5, ok: true });
