...
export const batchSyncTool: ToolDescriptor<BatchSyncInput, BatchSyncOutput> = {
  name: "forkable_batch_sync",
  description:
    "Sync multiple forks in sequence (rate-limit-friendly). Stops after `failFastAfter` consecutive failures (conflict or error). Conflicts are surfaced as 'conflict' outcomes — never force-pushed. Use forkable_propose_sync_pr to resolve.",
  ...
  try {
    ...
    if (e.code === "GITHUB_CONFLICT") {
      return {
        ...
        outcome: "conflict",
        message: "Diverged — fast-forward not possible. Use forkable_propose_sync_pr.",
      };
    }
    return {
      fork: ref,
