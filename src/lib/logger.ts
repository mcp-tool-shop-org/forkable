...
/**
 * Minimal structured logger for forkable.
 *
 * Ground rules:
 ...
 *     via a log line.
 *   - Env-controlled via FORKABLE_LOG = debug | info | warn | error | off.
...
/** No-op logger used when FORKABLE_LOG=off. */
...
/**
 * Build a logger honoring FORKABLE_LOG env. Pass the default level you want
...
export function buildLogger(opts: {
  ...
  const level = parseLevel(env.FORKABLE_LOG, opts.defaultLevel);
