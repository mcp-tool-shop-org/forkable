 ...
 * the DB in a half-migrated state. On failure we surface a ForkableError
...
export function runMigrations(
  ...
  if (fromVersion > toVersion) {
    throw new ForkableError(
      "INTERNAL",
      `State DB is at schema v${fromVersion} but this forkable build only knows up to v${toVersion}. You likely ran a newer forkable against this state dir and are now running an older one.`,
      {
        hint: "Upgrade forkable (`npm i -g @mcptoolshop/forkable`) or point FORKABLE_STATE_DIR at a fresh directory.",
  ...
  for (let v = fromVersion + 1; v <= toVersion; v++) {
    ...
    if (!migration) {
      throw new ForkableError(
        ...
        {
          hint: "This is a forkable bug — every version between the old and new schema must have a migration entry. File an issue at https://github.com/mcp-tool-shop-org/forkable/issues.",
